...
public class Buy extends AbstractGENode {
        ...
        if(withdrawnGP){
            ...
            if(inventory.getAmount(995) >= 300000){ //withdraw ge and check if enough
                ...
                if (operations.buyUpToLimit(nextRecipe.getPrimary(), buyPrice, 1000)) {
                    lastSuccess = "offer placed";
                    if(waitForBuy(nextRecipe)){
    ...
    boolean waitForBuy(CombinationRecipes recipe) throws InterruptedException {
        int timer = 0;
        boolean lock = true;
    ...
    public boolean resubmitHigherOffer(CombinationRecipes recipe) throws InterruptedException {
        GrandExchange.Box buyingBox = findPrimaryIngredientBuyingBox();
        if(buyingBox != null){
            int amtLeftToBuy = operations.getAmountRemaining(buyingBox);
            if(operations.abortOffersWithItem(recipe.getPrimary())){
                int newBuyPrice = margins.findPrimaryIngredientMargin(recipe)[1];
                int[] newMargin = margins.getCachedConversionMargin(recipe);
                ...
                if(delta >= Margins.switchRecipeIfLower){ //only increase the offer if the new margin doesn't go under the switch threshold
                    ...
                    return operations.buyUpToLimit(recipe.getPrimary(), newBuyPrice, amtLeftToBuy);
                }
